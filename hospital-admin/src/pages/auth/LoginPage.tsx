...
const LoginPage: React.FC = () => {
  ...
  // Check for impersonation query param (from Super Admin)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const hospitalId = params.get('hospital_id');
    
    // If hospital_id is present, user is being impersonated
    // The backend should have set cookies, so we can check auth status
    if (hospitalId && user && status === 'authed') {
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center p-4">
        <div className="w-full max-w-md space-y-6 rounded-xl bg-background-light/90 dark:bg-background-dark/90 p-8 shadow-soft backdrop-blur-sm">
          ...
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-text-light/70 dark:text-text-dark/70">Sign in to your hospital admin account</p>
          </div>
