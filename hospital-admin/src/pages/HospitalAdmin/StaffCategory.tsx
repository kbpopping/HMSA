...
/**
 * Staff Category Page - Shows staff members of a specific category
 * 
 * Features:
 * - Table view filtered by category/role
 * - Search functionality
 * - Add staff member to this category
 * - Clickable names to view staff profile
 */
const StaffCategory = () => {
  ...
  // Convert route format back to role name (e.g., "support-staff" -> "Support Staff")
  const roleName = useMemo(() => {
    if (!category) return '';
    // Handle special cases - exact matches
  ...
  const categoryStaff = useMemo(() => {
    return allStaff.filter((member) => member.role === roleName);
  }, [allStaff, roleName]);

  // Filter by search query
  const filteredStaff = useMemo(() => {
    if (!searchQuery.trim()) return categoryStaff;
    
    const query = searchQuery.toLowerCase();
    return categoryStaff.filter(
      (member) => {
  ...
  }, [categoryStaff, searchQuery]);

  // Handle row click - navigate to staff profile
  const handleRowClick = (row: Clinician) => {
    if (!category) return;
    navigate(`/hospital/staff/${category}/${row.id}`);
  };
        ...
        <div className="max-w-7xl mx-auto">
          <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg p-4">
            <p className="text-red-700 dark:text-red-300">
              Invalid category. Please select a valid staff category.
            </p>
          </div>
        </div>
      </AppShell>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={`Search ${roleName.toLowerCase()} by name, email, specialty, or phone...`}
              value={searchQuery}
        ...
        {error ? (
          ...
          <div className="bg-card-light dark:bg-card-dark rounded-xl shadow-soft overflow-hidden">
            <DataTable
              ...
              columns={columns}
              isLoading={isLoading}
              emptyMessage={`No ${roleName.toLowerCase()} found`}
              emptyIcon="people"
        ...
        >
          <form onSubmit={handleCreateSubmit} className="space-y-4">
            <TextField
              ...
              placeholder={roleName === 'Clinician' ? 'Dr. John Doe' : 'John Doe'}
              required
            />
            <div>
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              >
                {createMutation.isPending ? 'Adding...' : `Add ${roleName}`}
              </button>
            </div>
          </form>
        </Modal>
